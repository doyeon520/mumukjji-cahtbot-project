...
def feel():
    ...
    def feelings(tx):
        ...
        for record in result1:
            lst.append(record['화남단어'])
        for record in result2:
            lst.append(record['지침단어'])
        for record in result3:
            lst.append(record['슬픔단어'])

    df4 = run_query(feelings)
    return lst

def get_levenshtein(word,word_list):
    s1 = word
    diction={}
    for i in word_list:
        lv=jamo_levenshtein(s1, i)
        if lv <= 1:
            diction[i] = lv
            #print(diction)
    #         print(i, ':' ,lv)
    print(len(diction))
    if len(diction) > 0:
        key_min=min(diction.keys(),key=(lambda k: diction[k]))
    else:
        key_min=None
    return key_min
